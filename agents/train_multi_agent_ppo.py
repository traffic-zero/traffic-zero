...
class MultiAgentPPOWrapper(gym.Env):
    ...
    def __init__(
        self, multi_agent_env: MultiAgentTrafficEnv, config: dict[str, Any]
    ):
        """
        Initialize wrapper.
        ...
        Args:
            ...
            config: Configuration dictionary with action_config
        """
        super().__init__()
        ...
        self.num_phases = action_config.get("num_phases", 4)
        min_duration = action_config.get("min_duration", 10)
        max_duration = action_config.get("max_duration", 90)
        duration_step = action_config.get("duration_step", 10)
        ...
        # Action space: [phase, duration_idx] per agent
        # Each agent has num_phases choices for phase
        # and num_duration_steps for duration
        self.action_space = spaces.MultiDiscrete(
            [self.num_phases, self.num_duration_steps] * len(self.agent_ids)
        )
    ...
    def step(
        ...
        Action format: [phase1, dur1, phase2, dur2, ...] flattened
        Each agent gets (phase, duration_idx) pair.
        """
        num_agents = len(self.agent_ids)
        # Reshape to (num_agents, 2) for (phase, duration) pairs
        action_flat = np.asarray(action).flatten()
        action_per_agent = action_flat.reshape(num_agents, 2)

        # Build action dict: each agent gets (phase, duration_idx) tuple
        actions_dict = {
            agent_id: (int(action_per_agent[i][0]), int(action_per_agent[i][1]))
        ...
        # Average rewards across agents to keep in reasonable range
        num_agents = len(rewards_dict)
        shared_reward = sum(rewards_dict.values()) / max(num_agents, 1)
...
def make_env(
    ...
    wrapped = MultiAgentPPOWrapper(env, config)
...
def evaluate(
    ...
    for episode in range(n_episodes):
        ...
        env = make_env(sumo_cfg_file, config, gui=gui)
...
def train(
    ...
    """
    Train multi-agent PPO agents (training only, no evaluation).
    ...
    Args:
        ...
        checkpoint_freq: Checkpoint save frequency in timesteps
    """
    ...
    # Force CPU device for MlpPolicy training
    model = PPO(
        "MlpPolicy",
        env,
        ...
        device="cpu",  # Force CPU training for MlpPolicy
        verbose=1,
    )
    ...
    # Build callback list (no evaluation during training)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help="Checkpoint save frequency in timesteps (default: 10000)",
    )
    ...
    if args.train_only:
        # Train-only mode: train without evaluation
        train(
        ...
        # Eval-only mode: run GUI evaluation with existing model
        if model_path:
        ...
        # Default mode: train first, then run GUI evaluation
