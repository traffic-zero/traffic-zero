...
def analyze_lane_data(csv_path: Path):
    ...
    if len(rows_with_vehicles) > 0:
        ...
        print(
            "Occupancy range: "
            + str(rows_with_vehicles["occupancy"].min())
            + " - "
            + str(rows_with_vehicles["occupancy"].max())
        )
        print(
            "Density range: "
            + str(rows_with_vehicles["density"].min())
            + " - "
            + str(rows_with_vehicles["density"].max())
        )
        print(
            "Vehicle count range: "
            + str(rows_with_vehicles["vehicle_count"].min())
            + " - "
            + str(rows_with_vehicles["vehicle_count"].max())
        )
        print(
            "Mean speed range: "
            + str(rows_with_vehicles["mean_speed"].min())
            + " - "
            + str(rows_with_vehicles["mean_speed"].max())
        )
        print(
            "Waiting time range: "
            + str(rows_with_vehicles["waiting_time"].min())
            + " - "
            + str(rows_with_vehicles["waiting_time"].max())
        )
        print(
            "Queue length range: "
            + str(rows_with_vehicles["queue_length"].min())
            + " - "
            + str(rows_with_vehicles["queue_length"].max())
        )
        ...
        print(
            "Occupancy values > 0: "
            + str(len(rows_with_vehicles[rows_with_vehicles["occupancy"] > 0]))
        )
        print(
            "Occupancy values > 1: "
            + str(len(rows_with_vehicles[rows_with_vehicles["occupancy"] > 1]))
        )
        print(
            "Occupancy values > 100: "
            + str(
                len(rows_with_vehicles[rows_with_vehicles["occupancy"] > 100])
            )
        )
        print(f"\nMax occupancy: {rows_with_vehicles['occupancy'].max():.4f}")
        print("NOTE: SUMO occupancy is typically 0-100% (percentage)")
        print("If max is > 100, it might be in wrong units")
        print("If max is < 1, it might already be a fraction (0-1)")
        print("If max is 1-100, it's likely in percentage format")
        ...
        print("=" * 60)
        lane_max_occupancy = (
            rows_with_vehicles.groupby("lane_id")["occupancy"]
            .max()
    ...
    print("Rows in steps 0-50: " + str(len(early_steps)))
    print(
        "Vehicles in steps 0-50: "
        + str(len(early_steps[early_steps["vehicle_count"] > 0]))
    )
    if len(early_steps[early_steps["vehicle_count"] > 0]) == 0:
        print(
            "[INFO] No vehicles in early steps - "
            "this is normal if flows start later"
        )
        print(
            "First vehicle step: "
            + str(
                rows_with_vehicles["step"].min()
                if len(rows_with_vehicles) > 0
                else "N/A"
            )
        )
