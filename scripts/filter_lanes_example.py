#!/usr/bin/env python3
"""
Simple example of filtering lane data to only show lanes with non-zero occupancy/density.

This script demonstrates how to filter lane data to focus on lanes with actual traffic.
"""
...
def filter_lanes_simple(csv_path: Path, output_path: Path = None):
    """
    ...
    print(f"\nAfter filtering to entry/exit lanes (e*): {len(df_filtered)} rows")
    ...
    print(f"After filtering to non-zero occupancy/density: {len(df_non_zero)} rows")
    
    # Show summary
    print(f"\nFiltered data summary:")
    print(f"  Unique lanes: {df_non_zero['lane_id'].nunique()}")
    print(f"  Unique steps: {df_non_zero['step'].nunique()}")
    print(f"  Occupancy range: {df_non_zero['occupancy'].min():.2f} - {df_non_zero['occupancy'].max():.2f}%")
    print(f"  Density range: {df_non_zero['density'].min():.2f} - {df_non_zero['density'].max():.2f} veh/km")
    print(f"  Vehicle count range: {df_non_zero['vehicle_count'].min()} - {df_non_zero['vehicle_count'].max()}")
    
    # Show lanes
    lanes = df_non_zero['lane_id'].unique()
    print(f"\nLanes with traffic: {sorted(lanes)}")
...
def filter_main_roads_only(csv_path: Path, output_path: Path = None):
    """
    Filter to only main roads (eN_0, eS_0, eW_0, eE_0, eN_out_0, eS_out_0, eW_out_0, eE_out_0).
    ...
    print(f"Main roads with traffic: {len(df_main_non_zero)} rows")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python filter_lanes_example.py data/rush_hour_random/lane_data.csv")
        sys.exit(1)
    ...
    if output_csv:
        main_roads_output = output_csv.parent / (output_csv.stem + "_main_roads.csv")
        print("\n" + "=" * 60)
