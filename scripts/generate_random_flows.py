...
def generate_flows(
    ...
    for _ in range(num_flows):
        ...
        if use_vehs_per_hour:
            ...
            flow = generate_random_flow(route, use_vehs_per_hour=True, vehs_per_hour=vehs_per_hour)
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output file path (default: experiments/simple4/rush_hour_random.yaml)",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use vehsPerHour instead of probability (more consistent, default: True)",
    )
    parser.add_argument(
        "--min-vehs-per-hour",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.90,
        help="Maximum probability for flows (if not using vehsPerHour, default: 0.90)",
    )
    parser.add_argument(
        "--min-probability",
        type=float,
        default=0.50,
        help="Minimum probability for flows (if not using vehsPerHour, default: 0.50)",
    )
    ...
    print(f"[INFO] Routes used: {len(set(f['route'] for f in scenario['traffic']['flows']))}")
    
    if args.use_vehs_per_hour:
        vehs_per_hour_values = [f.get('vehsPerHour', 0) for f in scenario['traffic']['flows'] if f.get('vehsPerHour')]
        if vehs_per_hour_values:
            print(f"[INFO] Vehicles per hour range: {min(vehs_per_hour_values):.0f} - {max(vehs_per_hour_values):.0f}")
            print(f"[INFO] Average vehicles per hour: {sum(vehs_per_hour_values)/len(vehs_per_hour_values):.0f}")
    else:
        prob_values = [f.get('probability', 0) for f in scenario['traffic']['flows'] if f.get('probability')]
        if prob_values:
            print(f"[INFO] Probability range: {min(prob_values):.4f} - {max(prob_values):.4f}")
    
    print(f"[INFO] Simulation duration: {SIMULATION_DURATION}s (1 hour)")
    print(f"\n[TIP] This scenario will create MASSIVE congestion and overflow!")
    print(f"[TIP] Adjust --num-flows and --max-vehs-per-hour to control traffic volume")
    print(f"[TIP] Using vehsPerHour provides more consistent vehicle spawning than probability")
