...
def generate_sumocfg(
    simulation_name: str, experiment_name: str | None = None
) -> bool:
    """
    ...
    if not base_dir.exists():
        sys.stderr.write(
            f"[ERROR] Simulation directory not found: {base_dir}\n"
        )
        return False
    ...
    if not net_file.exists():
        ...
        sys.stderr.write(
            "Run netconvert first or ensure network.net.xml exists\n"
        )
        return False
...
def main():
    ...
    parser.add_argument("simulation", help="Simulation name (e.g., simple4)")
