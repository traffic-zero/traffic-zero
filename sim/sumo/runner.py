...
def run_interactive(simulation_name: str, experiment_name: str | None = None):
    """
    ...
    Example:
        ...
        >>> This will run interactively - you can control the simulation
        manually
    ...
    if not os.path.exists(SUMO_CFG) or experiment_name is not None:
        if experiment_name is not None:
            print(
                f">>> Generating configuration with experiment: "
                f"{experiment_name}"
            )
        else:
...
def _run_simulation_loop(
    ...
    """Run the main simulation loop with traffic light control and data
    collection.
    """
    ...
    while step < max_steps:
        ...
        if step % PROGRESS_REPORT_INTERVAL == 0:
            ...
            if (
                should_report_metrics
                and data_collector is not None
                and metrics_calculator is not None
            ):
                ...
                if not dfs["vehicles"].empty:
                    ...
                    print(
                        ...
                        + str(metrics.get("average_waiting_time", 0))
                        + "s"
                    )
                    print(
                        ">>>   Throughput: "
                        + str(metrics.get("throughput", 0))
                        + " vehicles"
                    )

        _apply_adaptive_control(
            step,
            tls_id,
            tls_controller,
...
def _apply_adaptive_control(
    ...
    # For production use, lane IDs should be dynamically determined
    # from the network
    # or made configurable via parameters/config files.
    try:
        q_len_east_raw = traci.lane.getLastStepVehicleNumber(example_lane_id)
        if isinstance(q_len_east_raw, tuple):
            q_len_east = (
                int(q_len_east_raw[0]) if len(q_len_east_raw) > 0 else 0
            )
        else:
            q_len_east = int(q_len_east_raw)
...
def run_automated(
    ...
    This function launches SUMO (with or without GUI) with TraCI control,
    allowing your Python code to programmatically control traffic lights,
    ...
    Args:
        ...
        experiment_name (str, optional): Name of experiment scenario to use.
                                        If provided, generates routes and tls
                                        from scenario before running.
        collect_interval (int): Collect data every N simulation steps
                                (default: 1)
        output_dir (str, optional): Directory to save CSV files. If None,
                                   no files are written but data is still
                                   collected in-memory.
        ...
        gui (bool): Show SUMO-GUI window (default: False, runs headless
             with 'sumo')
        realism_mode (RealismMode, optional): RealismMode instance for sensor
                                             noise. If None, uses
                                             RealismMode(RealismLevel.NONE)

    Returns:
        Dictionary containing:
            - 'data': Dict of pandas DataFrames (vehicles, traffic_lights,
                     lanes, junctions, edges, simulation)
            - 'metrics': Dict of computed metrics
            - 'tls_controller': TLSController instance for accessing action log

    Example:
        >>> result = run_automated("simple4", collect_interval=10,
        ...                        output_dir="./output")
    ...
    Note:
        Requires the simulation files (nodes.nod.xml, edges.edg.xml,
        routes.rou.xml) to exist in the intersection folder. The function will
        automatically generate the network and configuration files if needed.
        If experiment_name is provided, routes and tls will be generated from
        the scenario.
    """
    base_dir = "./sim/intersections/" + simulation_name
    ...
    if enable_data_collection:
        print(
            ">>> Initializing data collection (interval: "
        ...
        # Set lane_filter='main_roads' to only collect main roads
        # (eN_0, eS_0, eW_0, eE_0, etc.)
        data_collector = DataCollector(
            collect_interval=collect_interval,
            output_dir=output_dir,
            exclude_empty_lanes=False,  # Set to True to skip empty lanes
            lane_filter=None,  # Options: None, 'entry_exit', 'junction',
            # 'main_roads'
