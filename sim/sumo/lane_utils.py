...
def discover_main_lanes_from_network(intersection_dir: Path) -> Set[str]:
    ...
    to edges defined in edges.edg.xml. It excludes junction lanes (starting with ':').
    
    Args:
        intersection_dir: Path to intersection directory (e.g., sim/intersections/simple4)
    ...
    try:
        ...
        # Get all edge IDs from the network
        # Edges that don't start with ':' are real edges (junction edges start with ':')
        edge_ids = set()
        ...
        # Get all lanes that belong to these edges
        # Lanes are in format: {edge_id}_{lane_index}
        # Junction lanes start with ':' and should be excluded
        main_lanes = set()
        for lane in root.findall('.//lane'):
            ...
            # Skip junction lanes (start with ':')
            if lane_id.startswith(':'):
                continue
            
            # Check if this lane belongs to one of our main edges
            # Lane ID format is typically: {edge_id}_{lane_index}
            # Parse the edge ID from the lane ID
            lane_parts = lane_id.rsplit('_', 1)
            if len(lane_parts) == 2:
                # Check if the numeric part is actually a lane index (0, 1, 2, etc.)
                if lane_parts[1].isdigit():
                    edge_id = lane_parts[0]
                    if edge_id in edge_ids:
                        main_lanes.add(lane_id)
            # Also handle case where lane ID matches edge ID exactly (rare but possible)
            elif lane_id in edge_ids:
                main_lanes.add(lane_id)
...
def get_main_lanes_for_intersection(intersection_name: str, intersection_base_dir: Optional[Path] = None) -> Set[str]:
    """
    ...
    try:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Neither network.net.xml nor edges.edg.xml found in {intersection_dir}\n"
                "Please generate network files first (run netconvert or generate_sumocfg)"
            )


def is_real_lane(lane_id: str, intersection_name: Optional[str] = None, intersection_base_dir: Optional[Path] = None) -> bool:
    """
...
def filter_lanes_by_intersection(
    ...
    intersection_base_dir: Optional[Path] = None,
    ...
    Args:
        ...
        intersection_base_dir: Base directory for intersections
        keep_main_only: If True, keep only main lanes; if False, keep all non-junction lanes
