...
class MetricsCalculator:
    ...
    def __init__(self, output_dir: str | None = None):
        ...
        self.agg_funcs: dict[
            str, Callable[[pd.Series], int | float | pd.Series]
        ] = {
            "mean": pd.Series.mean,
            "max": pd.Series.max,
            "min": pd.Series.min,
        ...
        # Mapping from schema-based names to legacy names for backward
        # compatibility
    ...
    def calculate_metrics(
        ...
        Returns:
            Dictionary of metric names to values (uses legacy names for
        ...
        for name, df in dfs.items():
            if df is not None and not df.empty:
                ...
                # Convert schema-based names to legacy names for backward
                # compatibility
                for schema_name, value in schema_metrics.items():
                    legacy_name = self._legacy_name_mapping.get(
    ...
    def _calculate_metric_value(
        self, col_series: pd.Series | pd.DataFrame, func
    ) -> float:
        """Calculate a single metric value from a series."""
        if isinstance(col_series, pd.DataFrame):
            return 0.0

        series_len = len(col_series)
        if series_len == 0:
            return 0.0
        ...
        if isinstance(is_na_result, bool) and is_na_result:
            return 0.0
        if hasattr(is_na_result, "__bool__"):
            try:
    ...
    def _calculate_metrics_from_schema(
        ...
        for col, aggs in schema.items():
            ...
            for agg in aggs:
                ...
                val = self._calculate_metric_value(col_series, func)
                metrics[f"{category}_{col}_{agg}"] = val
    ...
    def _calculate_vehicle_metrics(
        ...
        if "fuel_consumption" in vehicle_df.columns:
            metrics["total_fuel_consumption"] = vehicle_df[
                "fuel_consumption"
            ].sum()
            metrics["average_fuel_consumption"] = vehicle_df[
                "fuel_consumption"
            ].mean()
        else:
    ...
    def _calculate_lane_metrics(
        ...
        if "waiting_time" in lane_df.columns:
            ...
            metrics["average_lane_waiting_time"] = lane_df[
                "waiting_time"
            ].mean()
        else:
    ...
    def _calculate_simulation_metrics(
        ...
        if "vehicle_count" in simulation_df.columns:
            metrics["average_vehicle_count"] = simulation_df[
                "vehicle_count"
            ].mean()
    ...
    def print_metrics(self, metrics: dict[str, float]):
        ...
        if "max_queue_length" in metrics:
            print(
                f"Max Queue Length: {metrics['max_queue_length']:.0f} vehicles"
            )
        if "average_queue_length" in metrics:
            print(
                "Average Queue Length: "
                + str(metrics["average_queue_length"])
                + " vehicles"
            )
        ...
        if "average_lane_speed" in metrics:
            print(
                f"Average Lane Speed: {metrics['average_lane_speed']:.2f} m/s"
            )
        ...
        if "total_fuel_consumption" in metrics:
            print(
                ...
                + str(metrics["total_fuel_consumption"])
                + " ml"
            )
        ...
        if "simulation_duration" in metrics:
            print(
                f"Simulation Duration: {metrics['simulation_duration']:.2f} s"
            )
