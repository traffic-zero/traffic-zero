...
class RealismMode:
    ...
    def _should_fail_sensor(self, sensor_id: str) -> bool:
        ...
        Args:
            sensor_id: Unique identifier for the sensor (e.g., lane_id, vehicle_id)
    ...
    def _apply_jitter(self, value: float, min_value: float = None, max_value: float = None) -> float:
        """
        ...
        """
        if self.jitter_std == 0.0 or value == 0:
            return value
        
        # Calculate jitter as percentage of value
        jitter_percent = np.random.normal(0, self.jitter_std)
        jitter_percent = np.clip(jitter_percent, -self.max_jitter_percent, self.max_jitter_percent)
        ...
        if max_value is not None:
            noisy_value = min(noisy_value, max_value) if max_value is not None else noisy_value
    ...
    def apply_noise_to_position(
        ...
        x, y = position
        noisy_x = self.apply_noise_to_value(x, f"{sensor_id}_x", default_value=x)
        noisy_y = self.apply_noise_to_value(y, f"{sensor_id}_y", default_value=y)
...
def get_realism_level(level_str: str) -> RealismLevel:
    ...
    raise ValueError(
        ...
        f"Must be one of: {', '.join([l.value for l in RealismLevel])}"
    )
